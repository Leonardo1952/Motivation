...
class UserActivityTest {
    ...
    fun setup() {
        ...
        context.getSharedPreferences("Motivation", Context.MODE_PRIVATE).edit().clear().apply()
    }

    @Test
    fun test_user_activity_is_displayed() {
        onView(withId(R.id.edit_person_name)).check(matches(isDisplayed()))
        onView(withId(R.id.button_save)).check(matches(isDisplayed()))
    }

    @Test
    fun test_finish_activity_when_user_name_is_valid() {
        onView(withId(R.id.edit_person_name)).perform(
